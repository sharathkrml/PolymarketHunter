  ...
  onMessage: async (c: RealTimeDataClient, message: Message) => {
    if (message.payload.size && message.payload.price) {
      ...
      for (const user of userIds) {
        ...
        let triggerReason = "✅ **Triggered by**: Trade size threshold"
        ...
        try {
          ...
          const shareOnXUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(
            shareText
          )}&url=${encodeURIComponent(enciesTxUrl)}`
